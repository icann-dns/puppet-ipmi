  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      describe 'Change Defaults' do
        ...
        context 'netmask' do
          before { params.merge!(netmask: '255.255.255.192') }
          it { is_expected.to compile }
          it do
        ...
        context 'type' do
          before { params.merge!(type: 'static') }
          it { is_expected.to compile }
          it do
            is_expected.to contain_exec('ipmi_set_static_1').with(
              ...
              notify: ['Exec[ipmi_set_ipaddr_1]', 'Exec[ipmi_set_defgw_1]', 'Exec[ipmi_set_netmask_1]'],
            )
          end
          it do
            is_expected.to contain_exec('ipmi_set_ipaddr_1').with(
        ...
        context 'lan_channel' do
          before { params.merge!(lan_channel: 42) }
          it { is_expected.to compile }
          it do
            is_expected.to contain_exec('ipmi_set_static_42').with(
              ...
              notify: ['Exec[ipmi_set_ipaddr_42]', 'Exec[ipmi_set_defgw_42]', 'Exec[ipmi_set_netmask_42]'],
            )
          end
          it do
      ...
      describe 'check bad type' do
        ...
        context 'lan_channel' do
          before { params.merge!(lan_channel: true) }
          it { expect { subject.call }.to raise_error(Puppet::Error) }
        end
      end
    end
